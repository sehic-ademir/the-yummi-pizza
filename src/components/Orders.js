...
class Orders extends Component {
    ...
    async getOrders(){
        ...
        if(id) {
            let data = [];
            const getOrders = await fetch('https://the-yummi-pizza-sehic.herokuapp.com/public/api/orders/getOrders/' + id);
            ...
            for(let i = 0; i < ordersJson.length; i++){
                ...
                data[i]['order'] = [{}];
            const getSingleOrder = await fetch('https://the-yummi-pizza-sehic.herokuapp.com/public/api/orders/getOrderProducts/' + ordersJson[i]['id']);
            const singleOrderJson = await getSingleOrder.json();
                for(let j = 0; j < singleOrderJson.length; j++){
                    const getPizza = await fetch('https://the-yummi-pizza-sehic.herokuapp.com/public/api/pizza/one/' + singleOrderJson[j]['pizza_id']);
